<div wire:key="{{ $todo->id }}" class="todo mb-5 card px-5 py-6 bg-white col-span-1 border-t-2 border-blue-500 hover:shadow">
    <div class="flex justify-between space-x-2">
        <div class="flex">
            <input wire:click="toggle({{ $todo->id }})" class="mr-2" type="checkbox" {{ $todo->completed ? 'checked' : '' }}>
            @if ($editingTodoId == $todo->id)
                 <input wire:model="editingTodoName" type="text"
                    class="bg-gray-100 text-gray-900 text-sm rounded block w-full p-2.5">
                            
                @error('editingTodoName')<span class="text-red-500 text-xs block">{{ $message }}</span>@enderror
            @else
